<div>
    ...
    {{-- @elseif(isset($tour["tour_gallery_images"]))
        @foreach ($tour["tour_gallery_images"] as $imageIndex => $image){
            <div class="relative">
                <div class="upload-container h-[{{$height}}px] border-2 border-dashed border-gray-300 rounded-lg hover:border-[#e92929] transition-colors cursor-pointer flex items-center justify-center" onclick="document.getElementById('gallery-input-{{$imageIndex + 1}}').click()">
                        <div class="text-center">
                            <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 mb-2"></i>
                            <p class="text-sm text-gray-600">Upload Image {{$imageIndex+1}}</p>
                            <p class="text-xs text-gray-500">400x{{$height}}px recommended</p>
                        </div>
                </div>
                <img id="preview-{{$imageIndex + 1}}" src="{{asset("storage/" . session("image"))}}" class="hidden w-full h-[{{$height}}px] object-cover rounded-lg shadow-lg hover:shadow-xl transition-shadow cursor-pointer" onclick="document.getElementById('gallery-input-{{$imageIndex + 1}}').click()">
                <input type="file" id="gallery-input-{{$imageIndex + 1}}" name="gallery_image[{{$imageIndex}}]" accept="image/*" class="hidden" onchange="previewImage({{$imageIndex + 1}}, this)">
                <button type="button" id="remove-{{$imageIndex + 1}}" class="hidden absolute top-2 right-2 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-colors" onclick="removeImage({{$imageIndex + 1}})">
                        <i class="fas fa-times text-sm"></i>
                </button>
            </div>
        }
